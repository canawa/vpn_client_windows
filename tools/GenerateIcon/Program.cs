...
var pngPath = Path.GetFullPath(args.Length > 1
    ? args[1]
    : Path.Combine("src", "CoffeeManiaVPN", "Assets", "logo-white.png"));
...
foreach (var (size, png) in sizes.Zip(pngImages))
{
...
static DrawingGroup CreateLogoDrawing()
{
    ...
    var bean2 = new GeometryDrawing(Brushes.White, null, bean2Geometry);
...
static byte[] RenderPng(DrawingGroup drawing, int size)
{
